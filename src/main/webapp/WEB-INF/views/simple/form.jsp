...
<div class="wrap">
	<div class="container">
		<%
			...
			%>
		<form action="${path}/simple/submit.do" method="post" role="form"
			class="leaveform">
			...
			<%
				...
				%>
			<input type='text' name="<%=fp.getId()%>"
				class="form-control input-lg" id="<%=fp.getId()%>" />
	</div>
	<%
					}
				  }%>
	<button type="submit" class="btn btn-primary btn-lg" name="result"
		value="同意">
		<span class="glyphicon glyphicon-ok-sign"></span> 申请
	</button>
	<%
				} else  if(!"申请".equals(task.getName())){
				%>
	<form action="${path}/simple/submit.do" method="post" role="form"
		class="form-horizontal leaveform">
		<h2 class="form-signin-heading text-center"><%=task.getName()%></h2>
		<input type="hidden" name="taskId" value="<%=taskId%>">
		...
		<button type="submit" class="btn btn-primary btn-lg" name="result"
			value="同意">
			<span class="glyphicon glyphicon-ok-sign"></span> 同意
		</button>
		
			<button type="submit" class="btn btn-primary btn-lg" name="result"
			value="不同意">
			<span class="glyphicon glyphicon-ok-sign"></span> 不同意
		</button>
		
		<% 
							if(activityList!=null&&activityList.size()>0){
								%>
		<button type="submit" class="btn btn-default btn-lg" name="result"
			value="驳回">
			<span class="glyphicon glyphicon-remove-circle"></span> 驳回
		</button>
		...
		<button type="submit" class="btn btn-default btn-lg" name="result"
			value="转签">
			<span class="glyphicon glyphicon-remove-circle"></span> 转签
		</button>
		<%				
					}
					%>
